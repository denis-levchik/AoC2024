...
def search_path(wall_hash, current_best_points)
  start_position = [1,1]
...
current_best_points = search_path(wall_hash, nil)

file_data.each_with_index do |line, index|
  puts index
  column_index, line_index = line.split(',').map { |n| n.to_i + 1 }

  wall_hash[[line_index, column_index]] = nil
  map[line_index][column_index] = '#'

  best_points = search_path(wall_hash, current_best_points)

  if best_points == nil
    puts("#{column_index - 1},#{line_index - 1}")
    break
  end
end
